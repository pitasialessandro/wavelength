...
watch(custom, (v) => localStorage.setItem(KEY, JSON.stringify(v)), {
  deep: true,
});
...
try {
  pools.value = {
    ...pools.value,
    ...JSON.parse(localStorage.getItem(POOLS_KEY) ?? "{}"),
  };
} catch {}
watch(pools, (v) => localStorage.setItem(POOLS_KEY, JSON.stringify(v)), {
  deep: true,
});
...
function draw() {
  ...
  if (!deck.length)
    deck.push(...[...pool.value].sort(() => Math.random() - 0.5));
  card.value = deck.pop()!;
}
...
<template>
  <main
    class="min-h-dvh flex flex-col items-center justify-center gap-10 p-6 bg-neutral-100 text-neutral-900"
  >
    <button
    ...
    <fieldset class="flex gap-2 text-sm">
      <label
        class="rounded-full border border-neutral-300 px-4 py-1.5 cursor-pointer select-none has-checked:bg-neutral-900 has-checked:text-white has-checked:border-neutral-900"
      >
        <input v-model="pools.og" type="checkbox" class="sr-only" /> Originali
        ({{ cards.length }})
      </label>
      <label
        class="rounded-full border border-neutral-300 px-4 py-1.5 cursor-pointer select-none has-checked:bg-neutral-900 has-checked:text-white has-checked:border-neutral-900"
      >
        <input v-model="pools.custom" type="checkbox" class="sr-only" /> Le mie
        ({{ custom.length }})
      </label>
    </fieldset>

    <details class="w-full max-w-sm text-sm">
      <summary class="cursor-pointer text-neutral-500 text-center">
        Le mie carte ({{ custom.length }})
      </summary>
      <form class="mt-4 flex gap-2" @submit.prevent="add">
        ...
        <input
          ...
          class="min-w-0 flex-1 rounded-lg border border-neutral-300 bg-white px-3 py-2"
        />
        <button class="rounded-lg bg-neutral-900 text-white px-4 font-semibold">
          +
        </button>
      </form>
      <ul class="mt-3 divide-y divide-neutral-200">
        <li
          v-for="([l, r], i) in custom"
          :key="i"
          class="flex items-center gap-2 py-2"
        >
          <span class="flex-1"
            >{{ l }} <span class="text-neutral-400">↔</span> {{ r }}</span
          >
          <button
            class="text-neutral-400 px-2"
            aria-label="Elimina"
            @click="remove(i)"
          >
            ×
